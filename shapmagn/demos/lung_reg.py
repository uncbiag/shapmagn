...
def analysis(shape_pair, fea_to_map, mapped_fea, compute_on_half_lung=True, method_name="flowed",saving_path=None):
...
source, target = matching_radius(source, target,sampled_by_radius=False, show=False)
...
#
#
# task_name = "discrete_flow_debugging"
...
# solver_opt["capture_plot_obj"] = "lung_data_analysis.capture_plotter()"
# solver_opt["point_grid_scales"] =  [-1]
# solver_opt["iter_per_scale"] = [50] if not gradient_flow_mode else [10]
...
# kernel_size = 0.12
# spline_param="cov_sigma_scale=0.02,aniso_kernel_scale={},principle_weight=(2.,1.,1.),eigenvalue_min=0.1,iter_twice=True".format(kernel_size)
# if not use_aniso_kernel:
#     model_opt["spline_kernel_obj"] ="point_interpolator.nadwat_kernel_interpolator(scale={}, exp_order=2)".format(kernel_size)
# else:
#     model_opt["spline_kernel_obj"] ="point_interpolator.nadwat_interpolator_with_aniso_kernel_extractor_embedded(exp_order=2,{})".format(spline_param)
#     model_opt["get_anistropic_gamma"] ="local_feature_extractor.compute_anisotropic_gamma_from_points({})".format(spline_param)
...
# model_opt["pair_feature_extractor_obj"] ="lung_feature_extractor.lung_pair_feature_extractor(fea_type_list=['eigenvalue'],weight_list=[0.2], radius=0.03,include_pos=True)"
...
# model_opt["gradflow_guided"] ['gradflow_reach_min']= 0.3
# model_opt["gradflow_guided"] ['pair_shape_transformer_obj']="lung_data_analysis.pair_shape_transformer( init_thres= 2.9e-5, nstep=5)"
# model_opt["gradflow_guided"] [("geomloss", {}, "settings for geomloss")]
# model_opt["gradflow_guided"]["geomloss"]["attr"] = "pointfea" #todo  the pointfea will be  more generalized choice
...
# blur = 0.001
# model_opt['sim_loss']['geomloss']["geom_obj"] = "geomloss.SamplesLoss(loss='sinkhorn',blur={}, scaling=0.8,reach=1.0, debias=False)".format(blur)
#
# model = MODEL_POOL[model_name](model_opt)
# solver = build_multi_scale_solver(solver_opt,model)
# model.init_reg_param(shape_pair)
