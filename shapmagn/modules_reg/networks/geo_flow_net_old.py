...
class GeoFlowNet(nn.Module):
    def __init__(self, input_channel=1, initial_radius=0.001, initial_npoints=8192):
        ...
        self.spatial_conv0 = SpatialConv([radius*5, radius*10, radius*20], weight_list=[0.33,0.33,0.33]) # conv level0 from level0
        self.channel_conv0_2 = ChannelConv(16, 16)

        # level 1 down
        self.sampler1 = furthest_sampling(int(initial_npoints))
        radius = initial_radius*4
        self.spatial_conv1_0 = SpatialConv([radius*5, radius*10, radius*20], weight_list=[0.33,0.33,0.33]) # conv level1 from level0
        self.channel_conv1_0 = ChannelConv(16, 32)
        self.channel_conv1_1 = ChannelConv(32, 32)
        self.spatial_conv1_1 = SpatialConv([radius*5, radius*10, radius*20], weight_list=[0.33,0.33,0.33]) # conv level1 from level1
        ...
        self.spatial_conv2_0 = SpatialConv([radius * 5, radius * 10, radius * 20], weight_list=[0.33,0.33,0.33])  # conv level2 from level1
        ...
        self.spatial_conv3_0 = SpatialConv([radius * 5, radius * 10, radius * 20],
                                                     weight_list=[0.33,0.33,0.33])  # conv level3 from level2
        ...
        self.spatial_conv3_1 = SpatialConv([radius * 5, radius * 10, radius * 20],
                                                     weight_list=[0.33,0.33,0.33])  # conv level3 from level3, do twice
        ...
        self.spatial_conv2_2 = SpatialConv([radius * 5, radius * 10, radius * 20],
                                                     weight_list=[0.33,0.33,0.33])  # conv level2 from level3, do twice
        ...
        self.spatial_conv2_3 = SpatialConv([radius * 5, radius * 10, radius * 20],
                                                     weight_list=[0.33,0.33,0.33])  # conv level2 from level2, do twice
        self.channel_conv2_5 = ChannelConv(96, 96)

        # level 1 up
        radius = initial_radius * 4
        self.spatial_conv1_2 = SpatialConv([radius * 5, radius * 10, radius * 20],
                                                     weight_list=[0.33,0.33,0.33])  # conv level1 from level2
        ...
        self.spatial_conv0_2 = SpatialConv([radius * 5, radius * 10, radius * 20],
                                                     weight_list=[0.33,0.33,0.33])  # conv level0 from level1
