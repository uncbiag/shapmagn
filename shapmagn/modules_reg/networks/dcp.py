...
from shapmagn.modules_reg.networks.transformer import Transformer, Identity


class DCP(nn.Module):
    def __init__(
        self, feature_model=DGCNN(), cycle=False, pointer_="transformer", head="svd"
    ):
        ...
        if pointer_ == "identity":
            ...
            self.pointer = Transformer(
                self.emb_nn.emb_dims, n_blocks=1, dropout=0.0, ff_dims=1024, n_heads=4
            )
        else:
    ...
    def forward(self, template, source):
        ...
        source_features_p, template_features_p = self.pointer(
            source_features, template_features
        )
        ...
        rotation_ab, translation_ab = self.head(
            source_features, template_features, source, template
        )
        if self.cycle:
            rotation_ba, translation_ba = self.head(
                template_features, source_features, template, source
            )
        else:
...
class MLPHead(nn.Module):
    ...
    def forward(self, *input):
        ...
        return transform.quat2mat(rotation), translation
