    ...
    def __init__(self, initDict=None, printSettings=True):
        if initDict is not None:
            if type(initDict) == type(self):
                ...
                print(
                    "WARNING: Cannot initialize from non ParameterDict object. Ignoring initialization."
                )
                self.ext = {}
        else:
            self.ext = {}
    ...
    def __getitem__(self, key_or_keyTuple):
        ...
        if type(key_or_keyTuple) == tuple:
            ...
            if lT == 1:
                ...
                return self._get_current_key(key_or_keyTuple[0], key_or_keyTuple[1])
            elif lT == 3:
                # treat this as keyword + default value + comment
                return self._get_current_key(
                    key_or_keyTuple[0], key_or_keyTuple[1], key_or_keyTuple[2]
                )
            else:
    ...
    def _get_current_key(self, key, defaultValue=None, comment=None):
        ...
        if key in self.ext:
            ...
            if type(value) == dict:
                ...
                currentCategoryName = self.currentCategoryName + "." + str(key)
                newpar._set_value_of_instance(
                    self.ext[key], self.int[key], self.com[key], currentCategoryName
                )
...
def test_parameter_dict():
    ...
    p["registration_model"]["similarity_measure"] = (
        {},
        "settings for the similarity measures",
    )
    p["registration_model"]["similarity_measure"]["type"] = (
        "ssd",
