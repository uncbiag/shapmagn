...
def read_img(file_path, return_np=True):
    ...
    if return_np:
        return img_np, np.flipud(spacing_itk), np.array([0.0, 0.0, 0])
    else:
        return img_sitk, spacing_itk, np.array([0.0, 0.0, 0])
...
def read_axis_reversed_img(file_path, return_np=True):
    ...
    if return_np:
        ...
        img_sitk.SetOrigin(np.array([0.0, 0.0, 0]))
...
def process_points(point_path, img_path, case_id, output_folder, is_insp):
    ...
    print(
        "{} : {},".format(copd, np.array(COPD_spacing[copd]) / downsampled_spacing_itk)
    )
...
if get_dirlab_high_shape_info:
    for i, _id in enumerate(id_list):
        high_img_insp_path, high_img_exp_path = (
            high_img_insp_path_list[i],
            high_img_exp_path_list[i],
        )
        compare_dirlab_and_high_nrrd(
            high_pair_path=[high_img_insp_path, high_img_exp_path], case_id=_id
        )

high_to_dirlab_processed_folder = os.path.join(
    processed_output_path, "process_to_dirlab"
)
...
if map_high_to_dirlab:
    for i, _id in enumerate(id_list):
        ...
        process_high_to_dirlab(
            high_pair_path=[high_img_insp_path, high_img_exp_path],
            case_id=_id,
            saving_folder=high_to_dirlab_processed_folder,
        )


landmark_processed_folder = os.path.join(
    processed_output_path, "landmark_processed_colored"
)
os.makedirs(landmark_processed_folder, exist_ok=True)
if project_landmarks_from_dirlab_to_high:
    landmark_insp_physical_pos_list = [
        process_points(
            landmark_insp_path_list[i],
            high_img_insp_path_list[i],
            id_list[i],
            landmark_processed_folder,
            is_insp=True,
        )
        for i in range(len(id_list))
    ]
    landmark_exp_physical_pos_list = [
        process_points(
            landmark_exp_path_list[i],
            high_img_exp_path_list[i],
            id_list[i],
            landmark_processed_folder,
            is_insp=False,
