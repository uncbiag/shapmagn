...
def evaluate_3d(sf_pred, sf_gt):
    ...
    acc3d_strict = torch.logical_or(l2_norm < 0.05, relative_err < 0.05).view(B,-1).float().mean(-1)
    acc3d_relax = torch.logical_or(l2_norm < 0.1, relative_err < 0.1).view(B,-1).float().mean(-1)
    outlier = torch.logical_or(l2_norm > 0.3, relative_err > 0.1).view(B,-1).float().mean(-1)
...
def evaluate_2d(flow_pred, flow_gt):
    ...
    acc2d = torch.logical_or(epe2d < 3., relative_err < 0.05).float().view(B,-1).mean(-1)
...
def evaluate_res(is_kitti=False):
    def eval(metrics, shape_pair, batch_info,additional_param=None, alias=''):
        ...
        folder_path_list = [os.path.split(path)[0] for path in batch_info["source_info"]["data_path"]]
        tnp = lambda x: x.detach().cpu().numpy()
        flow_pred_2d_np, flow_gt_2d_np = geometry.get_batch_2d_flow(tnp(sp),tnp(tp),
                                                                    tnp(fp),folder_path_list,is_kitti)
        EPE2D, acc2d = evaluate_2d(torch.from_numpy(flow_pred_2d_np), torch.from_numpy(flow_gt_2d_np))
        metrics_update = {
