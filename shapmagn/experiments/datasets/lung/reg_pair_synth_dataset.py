...
class RegistrationPairDataset(Dataset):
    ...
    def __init__(self, data_root_path, option=None, phase=None):
        ...
        synth_data_obj = option["synth_data","","a function object that synthesize the shape"]
        self.synth_data = obj_factory(synth_data_obj)
        self.synth_pair_ratio = option["synth_pair_ratio",0.0,"from 0 to 1, if 0. then not use synth pair"]
    ...
    def get_synth_pair(self,idx):
        # print(idx)
        random_state = np.random.RandomState(int(time.time()))
        synth_from_source = random_state.rand() > 0.5

        idx = idx % len(self.pair_name_list)
        pair_info = self.pair_info_list[idx]["source"] if synth_from_source else self.pair_info_list[idx]["target"]
        pair_name = self.pair_info_list[idx]["source"]["name"] + "_and_its_synth" if synth_from_source else \
            self.pair_info_list[idx]["target"]["name"] + "_and_its_synth"
        source_info, target_info = pair_info["source"], pair_info["target"]
        source_info = target_info if not synth_from_source else source_info
        ...
        if not self.load_into_memory:
            ...
            zip_source_dict = zip_target_dict if not synth_from_source else zip_source_dict
        ...
        target_dict, synth_info = self.synth_data(source_dict)
        synth_info["synth"] = True
        ...
        sample = {"source": source_dict, "target": target_dict,
                  "pair_name": pair_name,
                  "source_info": source_info, "target_info": target_info,"synth_info":synth_info}
        return sample


    def get_custom_pair(self, idx):
        """
        get pair data_dict
    ...
    def __getitem__(self,idx):
        random_state = np.random.RandomState(int(time.time()))
        synth_from_source = random_state.rand() <self.synth_pair_ratio
...
if __name__ == "__main__":
    ...
    sampler_obj = "lung_dataloader_utils.lung_sampler(num_sample=1000, method='uniform')"
    # sampler_obj = "lung_dataloader_utils.lung_sampler(num_sample=1000, method='voxelgrid',scale=5)"
    normalizer_obj = "lung_dataloader_utils.lung_normalizer(scale=[100,100,100],shift=[50,50,50])"
