...
class FlowModel(nn.Module):
    def __init__(self, opt):
        ...
        self.dim = 3
        self.opt = opt
        self.model_type = opt[("model_type","liner_interp","model type 'liner_interp'/'lddmm_shooting'")]
        self.t_list = opt[("t_list",[1.],"time list")]
    ...
    def lddmm_interp(self, shape_pair):
        ...
        if shape_pair.dense_mode:
            ...
            _, flowed_control_points_list = self.integrator.solve((momentum, shape_pair.control_points))
            flowed_points_list = flowed_control_points_list
        else:
            self.lddmm_module.set_mode("flow")
            _, flowed_control_points_list, flowed_points_list = self.integrator.solve((momentum, shape_pair.control_points, toflow_points))
        flowed_list = [Shape().set_data_with_refer_to(flowed_points, toflow) for flowed_points in flowed_points_list]
        return flowed_list
...
def visualize_animation(shape1_list, shape2_list, title1_list, title2_list, rgb_on=True, saving_capture_path_list=None, camera_pos_list=None,show=False):
    from shapmagn.utils.visualizer import color_adaptive
    from shapmagn.utils.visualizer import format_input
    for shape1, shape2, saving_capture_path, title1, title2, camera_pos in zip(shape1_list, shape2_list,saving_capture_path_list, title1_list, title2_list, camera_pos_list):
...
for case_id in case_id_list:
    ...
    prealign_reg_param_path = os.path.join(folder_path,  case_id + "_prealigned_reg_param.npy")
    ...
    os.makedirs(output_folder, exist_ok=True)
    stage_name = "preview"
    camera_pos_start =[(2.195036914518257, 6.095982604001324, 1.93352845755372),
 (0.0, 0.0, 0.0),
 ...
 # (0.6703830032209878, 0.10782789341316827, 0.7341387977722521)]
    camera_pos_end = [(-4.924379645467042, 2.17374925796456, 1.5003730890759344), (0.0, 0.0, 0.0),
    ...
    saving_capture_path_list = [os.path.join(output_folder, "t_{:.2f}.png").format(t) for t in t_list]
    ...
    camera_pos_start=[(-4.924379645467042, 2.17374925796456, 1.5003730890759344),(0.0, 0.0, 0.0),(0.40133888001174545, 0.31574165540339943, 0.8597873634998591)]
    camera_pos_end=[(-4.924379645467042, 2.17374925796456, 1.5003730890759344),(0.0, 0.0, 0.0),(0.40133888001174545, 0.31574165540339943, 0.8597873634998591)]
    ...
    lddmm_opt["variational"]["kernel"] = "keops_kernels.LazyKeopsKernel(kernel_type='multi_gauss', sigma_list=[0.03,0.06,0.09],weight_list=[0.2,0.3,0.5])"
    ...
    shape_pair = create_shape_pair(prealigned, target,pair_name=case_id,n_control_points=-1)
    ...
    flow_model = FlowModel(flow_opt)
    flowed_list= flow_model(shape_pair)
    camera_pos_start = [(-4.924379645467042, 2.17374925796456, 1.5003730890759344), (0.0, 0.0, 0.0),
                        (0.40133888001174545, 0.31574165540339943, 0.8597873634998591)]
    camera_pos_end = [(-4.924379645467042, 2.17374925796456, 1.5003730890759344), (0.0, 0.0, 0.0),
                      (0.40133888001174545, 0.31574165540339943, 0.8597873634998591)]
    pos_interp_list = camera_pos_interp(camera_pos_start, camera_pos_end, flow_opt["t_list"])
    ...
    target_list = [target]*len(flow_opt["t_list"])
    flow_model = FlowModel(flow_opt)
    shape_pair = create_shape_pair(nonp, nonp_gf,pair_name=case_id,n_control_points=-1)
