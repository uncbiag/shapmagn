...
def flyingkitti_nonocc_pair_postprocess(flying3d=True):
    def flying3d_postprocess(source_dict, target_dict):
    ...
    def kitti_postprocess(source_dict, target_dict):
        npoints = source_dict["points"].shape[0]
        near_mask = np.logical_and(source_dict["points"][:,2] < DEPTH_THRESHOLD,target_dict["points"][:,2] < DEPTH_THRESHOLD)
