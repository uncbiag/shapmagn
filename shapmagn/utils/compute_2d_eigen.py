import torch
def compute_2d_eigen(cov,eigenvectors=True, flatten_output=True,descending_eigenvals=True):
    """
    ...
    if eigenvectors:
        vec = torch.stack([(val1-cov[:,3]),(val2- cov[:,3]),cov[:,2],cov[:,2]],1) # Bx4xHxW  the eigenvectors are not normalized
    if flatten_output:
        val = val.permute(0,2,3,1).contiguous().view(-1,2)
        vec = vec.permute(0,2,3,1).contiguous().view(-1,2,2) if vec is not None else None
    else:
