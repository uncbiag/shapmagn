...
class PointNetLK(nn.Module):
    ...
    def forward(self, template, source, maxiter=10):
        template, source, template_mean, source_mean = data_utils.mean_shift(
            template, source, self.p0_zero_mean, self.p1_zero_mean
        )

        result = self.iclk(template, source, maxiter)
        result = data_utils.postprocess_data(
            result,
            template,
            source,
            template_mean,
    ...
    def iclk(self, template, source, maxiter):
        ...
        # re-calc. with current modules_reg
        template_features = self.pooling(
            self.feature_model(template)
        )  # [B, N, 3] -> [B, K]
        ...
        for itr in range(maxiter):
            ...
            transformed_source = self.transform(
                est_T.unsqueeze(1), source
            )  # [B, 1, 4, 4] x [B, N, 3] -> [B, N, 3]
            source_features = self.pooling(
                self.feature_model(transformed_source)
            )  # [B, N, 3] -> [B, K]
    ...
    def handle_batchNorm(self, template, source):
        ...
        if training:
            # first, update BatchNorm modules_reg
            template_features, source_features = (
                self.pooling(self.feature_model(template)),
                self.pooling(self.feature_model(source)),
            )
