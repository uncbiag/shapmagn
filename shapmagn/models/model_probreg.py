...
class ProRegOPT(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.opt = opt
        self.probreg_module = ProbReg(self.opt[("probreg",{},"settings for probreg module")])
    ...
    def forward(self, shape_pair):
        ...
        shape_pair.set_control_points(shape_pair.source.points.clone(),shape_pair.source.weights)
        flowed_points = self.probreg_module(shape_pair.source, shape_pair.target, return_tranform_param=False)
        end.record()
        torch.cuda.synchronize()
        print("{}, it takes {} ms".format(shape_pair.pair_name, start.elapsed_time(end)))
