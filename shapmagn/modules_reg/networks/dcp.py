...
class OTHead(nn.Module):
    ...
    def forward(self, *input):
        input = [_input.transpose(2,1).contiguous() for _input in input]
        src_embedding = input[0]
        tgt_embedding = input[1]
        src = input[2]
        tgt = input[3]
        ...
        blur = 0.01
        gemoloss_func = geomloss.SamplesLoss(loss='sinkhorn',blur=blur, scaling=0.8,reach=100, debias=False, backend="online",potentials=True)
        F_i, G_j = gemoloss_func( weight1[:,:,0], src_embedding, weight2[:,:,0], tgt_embedding)  # todo batch sz of input and output in geomloss is not consistent
