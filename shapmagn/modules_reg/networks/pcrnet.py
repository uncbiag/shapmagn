...
class iPCRNet(nn.Module):
    def __init__(self, feature_model=PointNet(), droput=0.0, pooling="max"):
        ...
        self.linear = [
            nn.Linear(self.feature_model.emb_dims * 2, 1024),
            nn.ReLU(),
            nn.Linear(1024, 1024),
            nn.ReLU(),
            nn.Linear(1024, 512),
    ...
    def spam(self, template_features, source, est_R, est_t):
        ...
        identity = (
            torch.eye(3).to(source).view(1, 3, 3).expand(batch_size, 3, 3).contiguous()
        )
        ...
        est_t = (
            torch.bmm(est_R_temp, est_t.permute(0, 2, 1)).permute(0, 2, 1) + est_t_temp
        )
        # update rotation matrix.
        est_R = torch.bmm(est_R_temp, est_R)

        source = transform.quaternion_transform(
            source, pose_7d
        )  # Ps' = est_R*Ps + est_t
        ...
        if max_iteration == 1:
            ...
            for i in range(max_iteration):
                est_R, est_t, source = self.spam(
                    template_features, source, est_R, est_t
                )

        result = {
            "est_R": est_R,  # source -> template
