...
def compute_local_fea(fea_type_list, raidus=1.):
    ...
    def _compute(points):
        ...
        fea_combined = (fea_combined-fea_combined.mean(1,keepdim=True))/fea_combined.std(1,keepdim=True)
        fea_combined = fea_combined.clamp(-2,2)
        return points, fea_combined, mass[...,0]
    return _compute
...
def feature_extractor(fea_type_list, radius=0.01):
    lung_feature_extractor = compute_local_fea(fea_type_list,radius,normalize=True)
    def extract(flowed, target):
        flowed.pointfea = lung_feature_extractor(flowed.points)
        target.pointfea = lung_feature_extractor(target.points)
...
if __name__ == "__main__":
    ...
    # fea_type_list = ["mass","dev","eignvalue","eignvector]
