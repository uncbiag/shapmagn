...
class PRNet(nn.Module):
    def __init__(
        ...
        if attention == "identity":
            ...
            self.attention = Transformer(
                emb_dims=self.emb_dims, n_blocks=1, dropout=0.0, ff_dims=1024, n_heads=4
            )
        else:
            raise Exception("Not implemented")
    ...
    def forward(self, *input):
        ...
        for i in range(self.num_iters):
            ...
            if self.input_shape == "bnc":
                src = transform.transform_point_cloud(
                    src.permute(0, 2, 1), rotation_ab_pred_i, translation_ab_pred_i
                ).permute(0, 2, 1)
            else:
                src = transform.transform_point_cloud(
                    src, rotation_ab_pred_i, translation_ab_pred_i
                )
        ...
        result = {
            "est_R": rotation_ab_pred,
            "est_t": translation_ab_pred,
            "est_T": transform.convert2transformation(
...
if __name__ == "__main__":
    ...
    rotation_ab_pred, translation_ab_pred, loss = model(
        src, tgt, rotation_ab, translation_ab
    )
