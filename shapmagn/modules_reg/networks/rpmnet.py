...
class RPMNet(nn.Module):
    ...
    def spam(self, xyz_template, norm_template, xyz_source, norm_source):
        ...
        self.affinity = self.compute_affinity(
            self.beta, feat_distance, alpha=self.alpha
        )

        # Compute weighted coordinates
        log_perm_matrix = sinkhorn(
            self.affinity, n_iters=self.num_sk_iter, slack=self.add_slack
        )
    ...
    def forward(self, template, source, max_iterations: int = 1):
        ...
        for i in range(max_iterations):
            weighted_template = self.spam(
                xyz_template, norm_template, xyz_source_t, norm_source_t
            )  # Finding better correspondences after each iteration.
            ...
            xyz_source_t, norm_source_t = se3_transform(
                transform.detach(), xyz_source, norm_source
        ...
        est_T = convert2transformation(
            transforms[max_iterations - 1][:, :3, :3],
            transforms[max_iterations - 1][:, :3, 3],
        )
        transformed_source = torch.bmm(
            est_T[:, :3, :3], source[:, :, :3].permute(0, 2, 1)
        ).permute(0, 2, 1) + est_T[:, :3, 3].unsqueeze(1)

        result = {
            "est_R": est_T[:, :3, :3],  # source -> template
