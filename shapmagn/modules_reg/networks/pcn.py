...
class PCN(torch.nn.Module):
    ...
    def encoder_2(self):
        ...
        layers = [self.conv3, self.relu, self.conv4]
        return layers
    ...
    def decoder(self):
        ...
        layers = [self.linear1, self.relu, self.linear2, self.relu, self.linear3]
        return layers
    ...
    def folding(self):
        ...
        layers = [self.conv5, self.relu, self.conv6, self.relu, self.conv7]
        return layers
    ...
    def fine_decoder(self):
        ...
        grid_feature = grid.repeat(
            [self.coarse_output.shape[0], self.num_coarse, 1]
        )  # Bx16384x2
        ...
        point_feature = torch.reshape(
            point_feature, (-1, self.num_fine, 3)
        )  # Bx16384x3
        ...
        feature = torch.cat(
            [grid_feature, point_feature, global_feature], dim=2
        )  # Bx16384x1029
    ...
    def decode(self):
        ...
        for idx, layer in enumerate(self.decoder_layers):
            output = layer(output)
        self.coarse_output = output.view(
            self.global_feature_v.shape[0], self.num_coarse, 3
        )
