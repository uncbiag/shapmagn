...
from shapmagn.modules_reg.networks.pointconv_util import PointConv, PointConvD, PointWarping3, UpsampleFlow3,PointWarping, UpsampleFlow, PointConvFlow, SceneFlowEstimatorPointConv
...
class PointConvSceneFlowPWC2_2(nn.Module):
    def __init__(self, input_channel=3, initial_input_radius=1.,initial_input_npoints=40000,first_sampling_npoints=2048,neigh_num=16,predict_at_low_resl=False, param_shrink_factor=1,use_aniso_kernel=False):
        ...
        self.flow0 = SceneFlowEstimatorPointConv(sbf(32 + 64), sbf(32),channels = sbf([128, 128]), mlp = sbf([128, 64]), neighbors=neigh_num, weightnet=neigh_num)
        ...
        self.flow1 = SceneFlowEstimatorPointConv(sbf(64 + 64), sbf(64),channels = sbf([128, 128]), mlp = sbf([128, 64]), neighbors=neigh_num, weightnet=neigh_num)
        ...
        self.flow2 = SceneFlowEstimatorPointConv(sbf(128 + 64), sbf(128),channels = sbf([128, 128]), mlp = sbf([128, 64]), neighbors=neigh_num, weightnet=neigh_num)
        ...
        self.flow3 = SceneFlowEstimatorPointConv(sbf(256), sbf(256), flow_ch=0,channels = sbf([128, 128]), mlp = sbf([128, 64]), neighbors=neigh_num, weightnet=neigh_num)
    ...
    def forward(self, xyz1, xyz2, color1, color2):
        ...
        if not self.predict_at_low_resl:
            ...
            up_flow0 = self.upsample(pc1_l0, pc1_l1,
                                     flow1,resol_factor=scaler(3)) # todo interpolate the resid flow and then add it with the interoplated prev flow
