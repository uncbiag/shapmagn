...
from shapmagn.modules_reg.networks.pointconv_util import PointConv, PointConvD, PointWarping3, UpsampleFlow3,PointWarping, UpsampleFlow, PointConvFlow, SceneFlowEstimatorPointConv3
from shapmagn.modules_reg.networks.pointconv_util import index_points_gather as index_points, index_points_group, Conv1d, square_distance
import time
...
class PointConvSceneFlowPWC2_6(nn.Module):
    def __init__(self, input_channel=3, initial_input_radius=1.,initial_input_npoints=40000,first_sampling_npoints=2048,neigh_num=16,predict_at_low_resl=False, param_shrink_factor=1,use_aniso_kernel=False, control_shift_factor=0.0):
        ...
        self.flow0 = SceneFlowEstimatorPointConv3(sbf(32 + 64), sbf(32),channels = sbf([128, 128]), mlp = sbf([128, 64]), neighbors=neigh_num, weightnet=neigh_num)
        ...
        self.flow1 = SceneFlowEstimatorPointConv3(sbf(64 + 64), sbf(64),channels = sbf([128, 128]), mlp = sbf([128, 64]), neighbors=neigh_num, weightnet=neigh_num)
        ...
        self.flow2 = SceneFlowEstimatorPointConv3(sbf(128 + 64), sbf(128),channels = sbf([128, 128]), mlp = sbf([128, 64]), neighbors=neigh_num, weightnet=neigh_num)
        ...
        self.flow3 = SceneFlowEstimatorPointConv3(sbf(256), sbf(256), flow_ch=0,channels = sbf([128, 128]), mlp = sbf([128, 64]), neighbors=neigh_num, weightnet=neigh_num)
    ...
    def forward(self, xyz1, xyz2, color1, color2):
        ...
        up_shiftflow2 = self.upsample(pc1_l2, pc1_l3, shift_flow3,resol_factor=scaler(5))
        ...
        feat2, resid_flow2, resid_fea_flow2, resid_shift_flow2 = self.flow2(pc1_l2, new_feat1_l2, cost2, up_flow2)
        ...
        up_shiftflow1 = self.upsample(pc1_l1, pc1_l2, shift_flow2,resol_factor=scaler(4))
        ...
        feat1, resid_flow1 , resid_fea_flow1, resid_shift_flow1= self.flow1(pc1_l1, new_feat1_l1, cost1,
                                  up_flow1)
        ...
        if not self.predict_at_low_resl:
            ...
            up_shift_flow0 = self.upsample(pc1_l0, pc1_l1,  shift_flow1,resol_factor=scaler(3))
            ...
            _, resid_flow0, resid_fea_flow0,resid_shift_flow0 = self.flow0(pc1_l0, new_feat1_l0, cost0, up_flow0)
            ...
            additional_param = {"control_points": (pc1_l0+shift_flow0*self.control_shift_factor).transpose(2, 1).contiguous(),
                                "control_points_idx": torch.arange(pc1_l0.shape[1]).repeat(pc1_l0.shape[0], 1),
                                "predict_at_low_resl": self.predict_at_low_resl,
                                "control_shift":(shift_flow0*self.control_shift_factor).transpose(2, 1).contiguous()}
            ...
            floweds = [flow.transpose(2, 1).contiguous() for flow in floweds]
            fps_pc1_idxs = [fps_pc1_l1, fps_pc1_l2, fps_pc1_l3]
            additional_param = {"control_points": (pc1_l1+shift_flow1*self.control_shift_factor).transpose(2, 1).contiguous(), "control_points_idx": fps_pc1_l1,
                                "predict_at_low_resl": self.predict_at_low_resl,
                                "control_shift":(shift_flow1*self.control_shift_factor).transpose(2, 1).contiguous()}
