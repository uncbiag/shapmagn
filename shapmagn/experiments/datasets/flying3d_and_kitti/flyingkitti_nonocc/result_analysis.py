...
def compute_std(data_list,name_list):
    for i,name in enumerate(name_list):
        print("the mean and  std  and median of the {}: is {} , {}".format(name, np.mean(data_list[i]), np.std(data_list[i])),np.median(data_list[i]))
...
def get_res_dic():
    ...
    data_dic['WBM(affine)'] = get_experiment_data_from_record(inc(),'/playpen-raid1/zyshen/data/lung_expri/opt_prealign_60000/records/lmk_diff_mean_records_detail.npy')
    data_dic['CPD(non-rigid)'] = get_experiment_data_from_record(inc(),'/playpen-raid1/zyshen/data/lung_expri/opt_cpd_20000/records/lmk_diff_mean_records_detail.npy')
    data_dic['RobOT(non-rigid)'] = get_experiment_data_from_record(inc(),'/playpen-raid1/zyshen/data/lung_expri/opt_gf_60000/records/lmk_diff_mean_records_detail.npy')
    data_dic['RobOTP(spline)'] = get_experiment_data_from_record(inc(),'/playpen-raid1/zyshen/data/lung_expri/model_eval/deep_feature_pointconv_dirlab_complex_aniso_15dim_normalized_60000/records/lmk_diff_mean_records_detail.npy')
    ...
    data_dic['DRobOT(disp)'] = get_experiment_data_from_record(inc(),'/playpen-raid1/zyshen/data/lung_expri/model_eval/deep_flow_prealign_pwc2_2_continue_60000/records/lmk_diff_mean_records_detail.npy')
    data_dic['DRobOT(spline)'] = get_experiment_data_from_record(inc(),'/playpen-raid1/zyshen/data/lung_expri/model_eval/deep_flow_prealign_pwc_spline_4096_new_60000_8192_aniso/records/lmk_diff_mean_records_detail.npy')
    data_dic['DRobOT(LDDMM)'] = get_experiment_data_from_record(inc(),'/playpen-raid1/zyshen/data/lung_expri/model_eval/deep_flow_prealign_pwc_lddmm_4096_new_60000_8192_aniso_adaptivecontrol/records/lmk_diff_mean_records_detail.npy')
    return data_dic
...
def get_performance():
    ...
    global  order
    order = -1
    task_folder = '/playpen-raid1/zyshen/data/flying3d_nonocc_test_on_kitti/model_eval/timing'
    task_name = "deepflow_official_8192_with_aug_kitti"
    data_dic[task_name] = get_experiment_data_from_record(inc(),os.path.join(task_folder, task_name,'records/EPE3D_records_detail.npy'))
...
def get_speed():
    ...
    order = -1
    task_folder = '/playpen-raid1/zyshen/data/flying3d_nonocc_test_on_kitti/model_eval/timing'
    batch_task_folder = '/playpen-raid1/zyshen/data/flying3d_nonocc_larger_kitti/model_eval/batch'
    task_name = "deepflow_official_8192_with_aug_kitti"
...
def scatter_plot(name_list, x,y, xy_value, title,fpth=None):
    ...
    cbar.ax.set_yticklabels(['10 MB', '30 MB', '0.1 GB', '0.3 GB', '1 GB', '3 GB', '10 GB'])
    for i, name in enumerate(name_list):
        if name == "PWC(im)":
...
name_list = ['PWC', 'PWC_batch', 'PWC(im)', 'PWC(im)_batch', 'PWC(dense)', 'PWC(dense)_batch', 'PWC(im&dense)', 'PWC(im&dense)_batch', 'DRobOT', 'DRobOT_batch', 'DRobOT(dense)', 'DRobOT(dense)_batch', 'spline_im_dense_2step', 'spline_im_dense_2step_batch', 'RobOT', 'RobOT_batch', 'RobOT(dense)', 'RobOT(dense)_batch',"FLOT","FLOT_batch"]
...
scatter_plot(name_list, res["performance"], res["time"],res["memory"],"Performance on KITTI",fpth=fpth)
