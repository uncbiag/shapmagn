...
class DeepModel(ModelBase):
    ...
    def initialize(self, opt,device, gpus=None):
        ...
        create_shape_from_data_dict = opt[
            ("create_shape_from_data_dict", "shape_pair_utils.create_shape_from_data_dict()",
             "generator func")]
    ...
    def init_learning_env(self, opt, device,gpus):
        method_name = opt[('method_name', "deep_predictor", "specific optimization method")]
    ...
    def set_input(self, input_data, device, phase=None):
        ...
        batch_info = {"file_name":input_data["file_name"],
                           "shape_info":input_data["shape_info"],
                            "phase":phase, "epoch":self.cur_epoch,
        ...
        if len(self.gpu_ids) == 1 and self.gpu_ids[0] != 0:
            ...
            a = LazyTensor(input_data["file_name"]["points"][:, :, None])
            a.sum(1)
    ...
    def save_visual_res(self, save_fig_on,input_data, eval_res, phase):
        ...
        if save_fig_on:
            self.capture_plotter(self.shape_folder_2d, "{}_epoch_{}".format(phase, self.cur_epoch),self.batch_info["file_name"], shape)
    ...
    def analyze_res(self, eval_res,cache_res=True):
        ...
        if self.external_analyzer:
            self.external_analyzer(shape, self.batch_info["pair_name"], self._model, self.record_path)
        if cache_res:
